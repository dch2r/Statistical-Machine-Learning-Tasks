...
def find_closest_centroids(X, centroids):
    ...
    for i in range(X.shape[0]):
        # Calculate squared Euclidean distance from example i to all centroids
...
def compute_centroids(X, idx, K):
    ...
    for i in range(K):
        # Get all data points assigned to centroid i
        points = X[idx == i]
        if len(points) > 0:
            # Compute mean of these points
            centroids[i] = np.mean(points, axis=0)
        else:
            # If no points assigned to centroid, keep it at zero or reinitialize
            centroids[i] = np.mean(X, axis=0)  # Alternative: set to overall mean
