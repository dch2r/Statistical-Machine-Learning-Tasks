...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # Create divisor term: 10000^(2i/d_model) for i in range(0, d_model/2)
    ...
    def forward(self, x):
        ...
        # Get positional encodings for the sequence length S
        pos_encoding = self.pe[:, :S, :]
        
        # Add positional encodings to input and apply dropout
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Reshape to (N, seq_len, num_heads, head_dim) and transpose to (N, num_heads, seq_len, head_dim)
        Q = Q.view(N, S, self.num_heads, self.head_dim).transpose(1, 2)  # (N, H, S, E/H)
        K = K.view(N, T, self.num_heads, self.head_dim).transpose(1, 2)  # (N, H, T, E/H)
        V = V.view(N, T, self.num_heads, self.head_dim).transpose(1, 2)  # (N, H, T, E/H)
        
        # Compute attention scores: (N, H, S, T)
        scores = torch.matmul(Q, K.transpose(-2, -1)) / self.scale
        
        # Apply attention mask if provided
        if attn_mask is not None:
            # Convert boolean mask to the same device and dtype as scores
            mask = attn_mask.unsqueeze(0).unsqueeze(1)  # (1, 1, S, T)
            mask = mask.to(scores.device)
            # Fill masked positions with -inf so they get 0 attention after softmax
        ...
        # Apply attention to values: (N, H, S, E/H)
        attended = torch.matmul(attn_weights, V)
        
        # Reshape back: (N, S, H, E/H) -> (N, S, E)
        attended = attended.transpose(1, 2).contiguous().view(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape to extract patches: (N, C, num_patches_h, patch_size, num_patches_w, patch_size)
        patches = x.unfold(2, self.patch_size, self.patch_size).unfold(3, self.patch_size, self.patch_size)
        
        # Rearrange dimensions: (N, num_patches_h, num_patches_w, C, patch_size, patch_size)
        patches = patches.permute(0, 2, 3, 1, 4, 5)
        
        # Flatten patches: (N, num_patches_h * num_patches_w, C * patch_size * patch_size)
        patches = patches.contiguous().view(N, -1, self.patch_dim)
