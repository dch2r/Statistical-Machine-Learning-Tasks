...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # 2. Project image features to same dimension as word embeddings
        # Expand image features to match sequence length for cross-attention
        image_features = self.visual_projection(features)  # (N, W)
        image_features = image_features.unsqueeze(1)  # (N, 1, W)
        # Repeat image features across sequence dimension for cross-attention
        memory = image_features.expand(-1, T, -1)  # (N, T, W)
        
        # 3. Create target mask to prevent attending to future tokens
        # Create a lower triangular mask (causal mask)
        tgt_mask = torch.tril(torch.ones(T, T))  # (T, T)
        tgt_mask = tgt_mask.bool()  # Convert to boolean
        
        # 4. Pass through transformer decoder
        # The decoder takes:
        # - tgt: caption embeddings (N, T, W)
        # - memory: image features (N, T, W)  
        # - tgt_mask: causal mask (T, T)
        decoder_output = self.transformer(caption_emb, memory, tgt_mask=tgt_mask)  # (N, T, W)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        patch_embeddings = self.patch_embed(x)  # (N, num_patches, embed_dim)
        
        # 2. Add positional encodings
        patch_embeddings = self.positional_encoding(patch_embeddings)  # (N, num_patches, embed_dim)
        
        # 3. Pass through transformer encoder (no mask needed for ViT)
        encoded_patches = self.transformer(patch_embeddings)  # (N, num_patches, embed_dim)
        
        # 4. Average pool across all patch vectors to get a single feature vector per image
        # We take the mean across the sequence dimension (patch dimension)
        pooled_features = torch.mean(encoded_patches, dim=1)  # (N, embed_dim)
        
        # 5. Pass through classification head to get logits
        logits = self.head(pooled_features)  # (N, num_classes)
